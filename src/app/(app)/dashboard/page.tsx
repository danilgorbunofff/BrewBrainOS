...
import {
  ...
  LucideAlertCircle, LucideActivity, LucideArrowRight,
} from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  let stats = { activeBatches: 0, fermenting: 0, conditioning: 0, totalTanks: 0, tanksInUse: 0, lowStockItems: 0 }
  let recentBatches: any[] = []
  let lowStockList: any[] = []
  let gravityData: number[] = []
  ...
  if (brewery) {
    ...
    recentBatches = batches.slice(0, 4)
    lowStockList = inventory.filter(i => i.current_stock <= (i.reorder_point || 0)).slice(0, 5)

    // Build gravity sparkline from readings
    const readings = readingsRes.data || []
    gravityData = readings
        ...
        {brewery && (
          <>
            ...
            <OnboardingChecklist
              ...
              hasInventory={stats.lowStockItems >= 0 && stats.totalTanks > 0}
            />
            ...
            {/* Voice Logger */}
            <div className="relative group rounded-2xl p-1 bg-[radial-gradient(circle_at_50%_50%,rgba(245,158,11,0.1),transparent_70%)] animate-in fade-in duration-1000">

...
test.describe('offline queue e2e', () => {
  test('flushes queued voice logs after the browser reconnects', async ({ browser, baseURL }) => {
    ...
    try {
      ...
      expect(await callOfflineSyncFixture(page, 'enqueueVoice')).toBe(2)
      await expect(page.getByTestId('offline-sync-queue-count')).toHaveText('2')
  ...
  test('allows only one tab to flush the same queued item', async ({ browser, baseURL }) => {
    ...
    try {
      ...
      await expect(firstPage.getByTestId('offline-sync-queue-count')).toHaveText('1')

...
export function Sidebar({ userEmail, breweryName, breweries, activeBreweryId }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed top-0 left-0 z-50 h-full w-[260px] bg-[#060606]/95 backdrop-blur-2xl border-r border-white/5 flex flex-col transition-transform duration-300 ease-out',
          'md:translate-x-0',
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-6 border-b border-white/5">
          ...
          >
            <LucideX className="h-4 w-4 text-zinc-500" />
          </button>
        </div>
        ...
        <div className="mx-3 mb-2">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/5 transition-colors group"
          >
            <LucideSearch className="h-3.5 w-3.5 text-zinc-700 group-hover:text-zinc-500" />
            <span className="text-xs text-zinc-700 group-hover:text-zinc-500 font-medium flex-1 text-left">Search…</span>
            <kbd className="text-[9px] font-bold text-zinc-800 bg-white/5 px-1.5 py-0.5 rounded border border-white/5">⌘K</kbd>
          </button>
        </div>
        ...
        <div className="px-3 pb-3 space-y-1 border-t border-white/5 pt-3">
          ...
          {/* User Pill */}
          <div className="flex items-center gap-3 px-3 py-3 mt-2 rounded-xl bg-white/[0.02] border border-white/5">
            <div className="h-8 w-8 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0">
              <span className="text-xs font-black text-primary">{initials}</span>
            </div>
            <div className="overflow-hidden flex-1">
              <p className="text-xs font-bold text-zinc-300 truncate">{userEmail}</p>
              <p className="text-[9px] font-black uppercase tracking-widest text-zinc-700">Active</p>
            </div>
            <ThemeToggle className="h-7 w-7 text-zinc-700" />
          </div>
        </div>
      </aside>

      {/* Mobile Bottom Tab Bar */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-[#060606]/95 backdrop-blur-2xl border-t border-white/5 px-2 pb-[env(safe-area-inset-bottom)]">

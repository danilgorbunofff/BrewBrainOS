...
export default async function ReportsPage() {
  ...
  // Calculate averages for server side KPI
  ...
  // Calculate grand totals using the server time (no impact on aggregate counts)
  const monthlyReportForServer = Object.values(monthlyData)

  const grandTotal = {
    totalBatches: allBatches.length,
    completedBatches: allBatches.filter(b => b.status === 'complete' || b.status === 'packaging').length,
    totalBBL: monthlyReportForServer.reduce((s, m) => s + m.estimatedBBL, 0),
    totalGallons: monthlyReportForServer.reduce((s, m) => s + m.estimatedBBL, 0) * 31, // 1 BBL = 31 Gallons
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="border-b border-border pb-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mt-2">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-black tracking-tighter text-foreground">Compliance Reports</h1>
                <p className="text-muted-foreground font-medium mt-1">TTB Form 5130.9 monthly production &amp; FSMA sanitation logs.</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={kpi.label} className="rounded-2xl p-4 border border-border bg-surface">
              <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-2">{kpi.label}</p>
              <p className={`text-3xl font-black tracking-tighter ${kpi.accent ? 'text-primary' : 'text-muted-foreground'}`}>
                {kpi.value}
              </p>
            </div>
          ))}
        ...
        <div className="rounded-2xl border border-primary/10 bg-primary/[0.02] p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-sm font-bold text-foreground">TTB Form 5130.9 — Brewer&apos;s Report of Operations</p>
            <p className="text-xs text-muted-foreground font-medium mt-1">
              Production is estimated as <span className="text-foreground font-bold">1 batch = 1 tank fill</span> using 
              your average tank capacity of <span className="text-primary font-bold">{avgTankCapacity.toFixed(1)} BBL</span>.
              Formula: <span className="font-mono text-muted-foreground">1 BBL = 31 US Gallons</span>.
        ...
        <TTBReportTable
          batches={allBatches}
          avgTankCapacity={avgTankCapacity}
        ...
        <div className="rounded-2xl border border-border bg-surface overflow-hidden">
          <div className="px-5 py-3 border-b border-border flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">FSMA / HACCP Sanitation Log Summary</p>
            </div>
            <span className="text-[10px] font-bold text-muted-foreground">{grandTotal.sanitationCount} total entries</span>
          </div>
          <div className="p-5">
            {sanitationLogs && sanitationLogs.length > 0 ? (
              <div className="space-y-2">
                {sanitationLogs.slice(0, 10).map(log => (
                  <div key={log.id} className="flex items-center justify-between p-3 rounded-xl bg-surface border border-border">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-foreground">
                          Tank cleaned
                          {log.notes && <span className="text-muted-foreground font-medium"> — {log.notes}</span>}
                        </p>
                        <p className="text-[10px] text-muted-foreground font-mono">Tank ID: {log.tank_id.slice(0, 8)}</p>
                      </div>
                    </div>

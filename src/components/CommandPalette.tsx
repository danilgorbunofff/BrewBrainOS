  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-full max-w-lg animate-in fade-in slide-in-from-top-4 duration-200">
        <Command className="rounded-2xl border border-white/10 bg-[#0c0c0c]/95 backdrop-blur-2xl shadow-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-4 border-b border-white/5">
            <LucideSearch className="h-4 w-4 text-zinc-600 shrink-0" />
            <Command.Input
              placeholder="Search pages, actions…"
              className="w-full h-14 bg-transparent text-zinc-200 placeholder-zinc-600 outline-none text-sm font-medium"
              autoFocus
            />
            <kbd className="hidden sm:inline-flex text-[9px] font-black uppercase tracking-widest text-zinc-700 bg-white/5 border border-white/10 px-2 py-1 rounded-md shrink-0">
              ESC
            </kbd>
          </div>

          <Command.List className="max-h-[300px] overflow-y-auto p-2">
            <Command.Empty className="py-8 text-center text-sm text-zinc-600 font-medium">
              No results found.
            </Command.Empty>

            <Command.Group heading="Navigation" className="[&_[cmdk-group-heading]]:text-[9px] [&_[cmdk-group-heading]]:font-black [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-widest [&_[cmdk-group-heading]]:text-zinc-700 [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-2">
              {pages.map((page) => (
                <Command.Item
                  key={page.href}
                  value={`${page.name} ${page.keywords}`}
                  onSelect={() => navigate(page.href)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-zinc-400 cursor-pointer transition-colors data-[selected=true]:bg-primary/10 data-[selected=true]:text-primary"
                >
          ...
          <div className="border-t border-white/5 px-4 py-2.5 flex items-center justify-between">
            <span className="text-[9px] font-black uppercase tracking-widest text-zinc-800">BrewBrain OS</span>
